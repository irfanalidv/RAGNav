...
def test_paper_page_routing_and_crossref_follow() -> None:
    ...
    """
    doc = Document(
        doc_id="pdf:paper.pdf", source="paper.pdf", metadata={"type": "pdf", "mode": "paper"}
    )
    ...
    fake = FakeLLMClient()
    idx = RAGNavIndex.build(
        documents=[doc], blocks=[b0, b1], llm=fake, build_vectors=False, edges=edges
    )
    r = RAGNavRetriever(index=idx, llm=fake)

    res = r.retrieve_paper(
